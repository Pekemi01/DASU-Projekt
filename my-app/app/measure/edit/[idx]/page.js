...
import { getMeasureById, editMeasureRatings } from "@/lib/actions/massnahmen.js";
import Link from "next/link";

/** @description Checkliste zur Bewertung der Sauberkeit eines Platzes. */
...
/** @description Liste der Risikokategorien für die Risikomatrix. */
const RISIKEN = [
    "Stolpern / Stürzen",
 ...
 * @param {number} basiswert - Ausgangswert vor der Maßnahme
 * @param {number} zielwert - Erwarteter Wert direkt nach Umsetzung
 * @param {number} aktuellerWert - Gemessener Wert nach dem Kontrollintervall
 * @returns {number} Resilienzwert zwischen 0 und 1 (oder größer)
 */
 ...
 * Formel: Re = scoreDelta / gesamtkosten — höher bedeutet effizienter
 * @param {number} gesamtkosten - Gesamtkosten der Maßnahme in €
 * @param {number} scoreDelta - Verbesserung in Prozentpunkten
 * @returns {number} Effizienzwert (Verbesserung pro €)
 */
...
export default function EditScorePage() {
    ...
    /**
     * @description Lädt die Maßnahme und füllt das Formular vor.
     * Bevorzugt raw_inputs (exakte Rohwerte), fällt andernfalls auf
     * die Rückrechnung aus den gespeicherten Scores zurück.
     */
    useEffect(() => {
        getMeasureById(id).then((data) => {
            ...
            if (data.raw_inputs) {
                const ri = typeof data.raw_inputs === "string" ? JSON.parse(data.raw_inputs) : data.raw_inputs;
                if (ri.sauberkeitsCheckboxen?.length === SICHERHEITSFRAGEN.length) setSauberkeitsCheckboxen(ri.sauberkeitsCheckboxen);
                if (ri.sozialeAkzeptanz !== undefined) setSozialeAkzeptanz(ri.sozialeAkzeptanz);
                if (ri.risikoMatrix?.length === RISIKEN.length) setRisikoMatrix(ri.risikoMatrix);
                if (ri.basiswert !== undefined) setBasiswert(ri.basiswert);
                if (ri.zielwert !== undefined) setZielwert(ri.zielwert);
                if (ri.aktuellerWert !== undefined) setAktuellerWert(ri.aktuellerWert);
                if (ri.kosten !== undefined) setKosten(ri.kosten);
                if (ri.scoreDelta !== undefined) setScoreDelta(ri.scoreDelta);
            } else {
                // Fallback: soweit möglich aus berechneten Werten zurückrechnen
    ...
    /**
     * @description Verarbeitet das Absenden des Edit-Formulars.
     * Baut FormData mit berechneten Scores und Rohwerten auf, ruft editMeasureRatings auf
     * und leitet bei Erfolg zurück zur Maßnahmen-Übersicht. Bei Fehler wird eine Rückmeldung angezeigt.
     */
    const handleFormularAbsenden = async (e) => {
        ...
        const result = await editMeasureRatings(id, formData);
    ...
    return (
        <main style={{ maxWidth: 700, margin: "40px auto", padding: "0 16px" }}>
            ...
            <h1>✏️ Bewertung bearbeiten: {name}</h1>
            ...
            <form onSubmit={handleFormularAbsenden}>
                ...
                <section style={{ marginTop: 32, marginBottom: 40 }}>
                    <button type="submit" disabled={wirdGespeichert} style={{
                        ...
                        {wirdGespeichert ? "Wird gespeichert…" : "Änderungen speichern"}
                    </button>
                </section>
            </form>
        </main>
